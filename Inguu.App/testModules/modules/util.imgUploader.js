/**
 * [ util.imgUploader.js v1.0.1 ]
 * @lastModifyTime:2015年1月14日 17:27:50
 * @deps：jQuery2.0+
 * @settings:
		url:'',									//上传的地址
		data:{},								//传递的参数
		...
		timeout: 800,							//循环上传的延迟时间[防止请求频率过快]
		onPreview:function(dataUrl,file){},		//选中文件后的预览回调 [参数：图片的base64编码数据，文件信息]
		filter: function(files){ return files},	//文件过滤 [自定义的文件过滤器必须返回一个过滤后的文件集合]
		onError:function(errorMsg,level,files){},//错误类别：文件类型、文件太大、文件太小、上传出错, 级别level：red/yellow/green/blue
		onStart:function(e, file){},			//文件开始上传时
		onSelected: function(files) {},			//文件选择后
		onProgress: function(e, file, currPercent,totalPercent) {},		//文件上传进度
		onSuccess: function(e,responseText,file) {},					//单个文件上传成功时，参数：事件，服务器返回值，文件信息
		onCancel: function(e, file){},			//取消上传时的回调
		onCompleted: function() {},				//全部文件上传完毕时
 ...
 * @PS:
 * 	图片上传为队列上传，文件一个一个上传。
 * 	图片上传的地址参数中会自动添加配置里为文件的生成的guid。
 * @log:
 * 	- 添加了timeout和onPreview()
