		...
		crollEvent:function(oThis){
			if(!oThis.loadingStep){
				...
				if(this.y>20 && oThis.options.canPullDown){
					//下拉刷新
					oThis.$downEl.show().html('<span class="mui-icon mui-icon-arrowthinup"></span> 释放更新');
					oThis.loadingStep=1;
					oThis.isPullUp = false;
					oThis.iScrollObj.refresh();
				}else if(this.y < (this.maxScrollY-20) && oThis.options.canPullUp){
					//上拉刷新
					oThis.$upEl.show().html('<span class="mui-icon mui-icon-arrowthindown"></span> 释放更新');
					oThis.loadingStep=1;
					oThis.isPullUp = true;
					oThis.iScrollObj.refresh();
				}
...
Page.preInit=function(){
		    				...
		    				success:function(res){
		    					...
		    					var jsonData = $.xml2json(res);
					 		 	...
					 		 	if(count>0 && jsonData.mould){
					 		 		...
					 		 		Utils.debounce(function(){oThis.iScrollObj.refresh();},400,false)();
					 		 	}else{
					 		 		oThis.$upEl.html('<span class="mui-icon mui-icon-arrowthinup"></span> 没有更多了...');
					 		 	}
		    				},
		    				error:function(){
	...
	var styleClickFn = function(){
		...
		console.log(url);
    	$.post(GLOBAL.reqURL.proxyURL,{url:url},function(res){
	 		var jsonData = $.xml2json(res);
			console.log(jsonData);
	...
	$.ajax({url:GLOBAL.reqURL.proxyURL,type:'POST',data:data,async:false,success:function(res){
		 ...
		 console.log(jsonData);
 		 if(jsonData.mouldtype && jsonData.mouldtype.length){
 		 	...
 		 	if(styles.length>3){
 		 		$styleBox.css('width',$styleBox.children().first().outerWidth(true)*styles.length);
 		 		//添加滚动效果
        ...
        $('#pic-list-box li').pep({
			...
			rest: function(ev, obj){//拖到指定位置后的回调函数
				if ( obj.activeDropRegions.length > 0 ) {
					...
					var res = that.autoScaleImg(boxsize.w,boxsize.h,size.w,size.h);
					...
					$('#i'+imgId+mark).pep({
						...
						drag:function(ev, obj){
							...
							if(matrix){
								...
								o.imgPosInfo['i-'+layerIndex].ImageX=pos[4];
								o.imgPosInfo['i-'+layerIndex].ImageY = pos[5];
							}else{
								o.imgPosInfo['i-'+layerIndex].ImageX=o.imgPosInfo['i-'+layerIndex].ImageY=0;
							}
    ...
    buildSubData:function(){
    	...
    	for(; i<len; i+=1){
    		if(layerModels[i]['IsPhoto']==='true'){
    			var index = layerModels[i]['Index'];
    			layerModels[i]['Src'] = imgsInfo['i-'+index]['Src'];
    			layerModels[i]['Type'] = 'photo';
    			layerModels[i]['ImageX'] = imgsInfo['i-'+index]['ImageX'];
    			layerModels[i]['ImageY'] = imgsInfo['i-'+index]['ImageY'];
    			layerModels[i]['ImageWidth'] = imgsInfo['i-'+index]['ImageWidth'];
    			layerModels[i]['ImageHeight'] = imgsInfo['i-'+index]['ImageHeight'];
    			layerModels[i]['BaseWidth'] = imgsInfo['i-'+index]['BaseWidth'];
    			layerModels[i]['BaseHeight'] = imgsInfo['i-'+index]['BaseHeight'];
    		}else{
    			layerModels[i]['Type'] = 'icon';
    		}
    	}
    ...
    subOrderEv:function(){
    	...
    	$('#sub-order-btn').on('click',function(){
    		...
    		$.post(GLOBAL.reqURL.orderSubURL,data,function(data){
    			...
    			// 判断是否提交成功
    			if(data.indexOf('|')>0){
    				alert('订单提交成功！EditID='+data.split('|')[1]);
    				self.resetCartList();
		    		self.resetOrderInfo();
		    		Page.index.init();
					// $.pageTransition.nextPage($('#view-page-cart'));
    			}else{
    				alert('订单提交失败：'+data.substring(0,20));
    			}
    		});
